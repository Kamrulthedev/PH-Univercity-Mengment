...
const localGuardianvalidationSchema = z.object({
    ...
    contectNO: z.string().min(1, { message: "Local guardian contact number is required" }),
...
const guardianvalidationSchema = z.object({
    ...
    fatherContectNo: z.string().min(1, { message: "Father contact number is required" }),
    ...
    matherContectNo: z.string().min(1, { message: "Mother contact number is required" }),
    matherOccupation: z.string().min(1, { message: "Mother occupation is required" }),
...
const studentvalidationSchema = z.object({
  ...
  emargecyContectNo: z.string().min(1, { message: "Emergency contact number is required" }),
  bloodGroup: z.enum(["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"], { message: "Invalid blood group" }).optional(),
  persentAddress: z.string().min(1, { message: "Present address is required" }),
  permenantAddress: z.string().min(1, { message: "Permanent address is required" }),
  gardians: guardianvalidationSchema,
  localGardian: localGuardianvalidationSchema,
  profileImg: z.string().optional(),
  isActive: z.enum(["active", "block"], { message: "Invalid status" }).default("active"),
});
