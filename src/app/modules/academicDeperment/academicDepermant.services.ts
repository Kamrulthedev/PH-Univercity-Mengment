...
const GetallAcademicDeperment = async() =>{
    const result = await AcademicDeperment.find();
    return result;
};

const getSingleAcademicDeperment = async(id:string) =>{
    const result = await AcademicDeperment.findById(id);
    return result;
};

const updateAcademicDeperment = async(id:string,  payload: Partial<TAcademicDeperment>)=>{
